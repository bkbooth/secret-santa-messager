...
if (!sendMessages)
  console.warn(
    "Sending messages is disabled. Please set environment variable SEND_MESSAGES=true"
  );
...
function fullName(gifter) {
  let fullName = gifter.firstName;
  if (gifter.lastName) fullName += ` ${gifter.lastName}`
  return fullName;
}
